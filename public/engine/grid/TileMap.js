 ...
 *   - per-cell height (0..CONFIG.height.max) stored alongside terrain
 *   - getHeight / setHeight / raiseAt / lowerAt mutators
 *   - serialize/deserialize round-trips heights
 *
...
export class TileMap {
    constructor(width = CONFIG.grid.width, height = CONFIG.grid.height) {
        ...
        this._heights = new Array(width * height).fill(0);
        this._nextId = 1;
    ...
    /* ── Elevation (Island Forge) ─────────────────────────────── */

    getHeight(gx, gy) {
        if (!this.inBounds(gx, gy)) return 0;
        return this._heights[gy * this.width + gx] | 0;
    }

    setHeight(gx, gy, h) {
        if (!this.inBounds(gx, gy)) return;
        const max = CONFIG.height?.max ?? 3;
        const clamped = Math.max(0, Math.min(max, h | 0));
        const idx = gy * this.width + gx;
        if (this._heights[idx] === clamped) return;
        this._heights[idx] = clamped;
        // Height changes affect both terrain Y offset (cache) and object
        // sit-on-tile Y offset, so bump both version stamps.
        this.terrainVersion++;
        this.objectsVersion++;
    }

    raiseAt(gx, gy) { this.setHeight(gx, gy, this.getHeight(gx, gy) + 1); }
    lowerAt(gx, gy) { this.setHeight(gx, gy, this.getHeight(gx, gy) - 1); }

    /** Returns a flat heights array (defensive copy). */
    getHeights() { return Array.from(this._heights); }

    /** Replace heights from a flat array. */
    setHeights(arr) {
        if (!Array.isArray(arr)) return;
        const want = this.width * this.height;
        // Pad / truncate so legacy or cross-version data still applies.
        for (let i = 0; i < want; i++) {
            this._heights[i] = (i < arr.length ? arr[i] : 0) | 0;
        }
        this.terrainVersion++;
        this.objectsVersion++;
    }
    ...
    serialize() {
        return {
            ...
            heights: Array.from(this._heights),
    ...
    deserialize(data, objectFactory) {
        ...
        this._heights = Array.isArray(data.heights) && data.heights.length === this.width * this.height
            ? Array.from(data.heights)
